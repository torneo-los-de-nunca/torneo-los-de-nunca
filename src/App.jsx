...
function App() {
  ...
  useEffect(() => {
    ...
    if (guardado) {
      setJugadores(JSON.parse(guardado));
    } else {
      fetch("/jugadores.json?cache=" + Date.now())
        .then((res) => res.json())
        .then((data) => {
          const conTotales = data.map((j) => {
            const semanasNorm = (j.semanas || []).map(normalizarSemana);
            const total = semanasNorm.reduce(
              (acc, s) => acc + (Number(s.valor) || 0),
              0
            );
            return { ...j, semanas: semanasNorm, total };
          });

          conTotales.sort((a, b) => b.total - a.total);

          setJugadores(conTotales);
          localStorage.setItem("jugadores", JSON.stringify(conTotales));
        })
...
async function handleSaveEvent() {
  ...
  // 2️⃣ Guardar en el estado local
  ...
  // 3️⃣ Actualizar puntos y flags en jugadores
  setJugadores((prev) => {
    const jugadoresActualizados = prev.map((jugador) => {
      const opciones = selectedPlayers[jugador.nombre];

      if (!opciones?.selected) return jugador;

      const puntosGanados = calcularPuntosDeJugador(opciones);

      const nuevasSemanas = [...jugador.semanas];
      const semanaActual = nuevasSemanas[semanaSeleccionada - 1] || {};

      nuevasSemanas[semanaSeleccionada - 1] = {
        ...semanaActual,
        valor: (semanaActual.valor || 0) + puntosGanados,
        oro: opciones.oro || false,
        doble: opciones.doble || false,
        triple: opciones.triple || false,
        penal1: opciones.penal1 || false,
        penal10: opciones.penal10 || false,
        nosuma: opciones.nosuma || false,
      };

      return {
        ...jugador,
        semanas: nuevasSemanas,
        total: jugador.total + puntosGanados,
      };
    });

    jugadoresActualizados.sort((a, b) => b.total - a.total);
